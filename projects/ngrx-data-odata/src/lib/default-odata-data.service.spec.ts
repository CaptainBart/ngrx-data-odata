...
import { DefaultODataDataService, DefaultODataDataServiceFactory } from './default-odata-data.service';
...
describe('DefaultDataService', () => {
  ...
  let service: DefaultODataDataService<Hero>;
  ...
  beforeEach(() => {
    ...
    httpUrlGenerator = new ODataHttpUrlGenerator(null as any);
    httpUrlGenerator.registerHttpResourceUrls({
      Hero: {
        entityResourceUrl: heroesUrl,
        collectionResourceUrl: heroesUrl,
      },
    });

    service = new DefaultODataDataService('Hero', httpClient, httpUrlGenerator);
  });
  ...
  describe('property inspection', () => {
    ...
    class TestService<T> extends DefaultODataDataService<T> {
      properties = {
    ...
    it('has expected single-entity url', () => {
      expect(service.properties.entityUrl).toBe(heroesUrl);
    });
  ...
  describe('#add', () => {
    ...
    it('should return expected hero with id', () => {
      ...
      const req = httpTestingController.expectOne(
        r => r.method === 'POST' && r.url === heroesUrl
      );
  ...
  describe('#update', () => {
    ...
    it('should add If-Match header with e-tag', () => {
      // Call service.update with an Update<T> arg
      const updateArg: Update<Hero> = {
        id: 1,
        changes: { id: 1, name: 'B', '@odata.etag': 'abcdef' },
      };
      ...
      // One request to PUT hero from expected URL
      const req = httpTestingController.expectOne(
        r => r.method === 'PUT' && r.url === heroUrlId1 && r.headers.get('If-Match') === 'abcdef'
      );
  ...
  describe('#upsert', () => {
    ...
    it('should throw when no entity given', () => {
      service.upsert(undefined as any).subscribe(
        heroes => fail('update succeeded when expected it to fail'),
        err => {
          expect(err.error).toMatch(/Upsert is not supported/);
        }
      );
...
describe('DefaultDataServiceFactory', () => {
  ...
  describe('(no config)', () => {
    it('can create factory', () => {
      const factory = new DefaultODataDataServiceFactory(http, httpUrlGenerator);
  ...
  describe('(with config)', () => {
    it('can create factory', () => {
      ...
      const factory = new DefaultODataDataServiceFactory(
        http,
        httpUrlGenerator,
        config
      );
